...
void ApplicationCallback() {
    ...
    if (ImGui::Checkbox("Normal Rendering", &normal_render)) {
        ApplyGlobalRenderSettings();
    }
    if (ImGui::Checkbox("Show Wireframe (Edges)", &show_edges)) {
        ApplyGlobalRenderSettings();
    }
    if (ImGui::Checkbox("Enable Anti-Aliasing (4x)", &enable_ssaa)) {
        polyscope::options::ssaaFactor = enable_ssaa ? 4 : 1;
